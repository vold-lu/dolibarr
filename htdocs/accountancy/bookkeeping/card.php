...
if ($action == 'create') {
	...
	*/
	...
	if (!empty($object->piece_num)) {
		...
		$head = array();
		$h = 0;
		$head[$h][0] = DOL_URL_ROOT."/accountancy/bookkeeping/card.php".'?piece_num='.((int) $object->piece_num).($mode ? '&mode='.$mode : '').($type ? '&type='.$type : '').($backtopage ? '&backtopage='.urlencode($backtopage) : '');
